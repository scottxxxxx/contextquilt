...
Denby on turnaround time once IP address is provided" carried
`value.owner = "Denby"` plus `owns` edges from BOTH Denby and
Ellery. The transcript is unambiguous that Denby does the whitelisting
and Ellery supplies the IP address it waits on. The app rendered both as
...
sanitizer does not need:
  ...
  - Compound owners ("Marlowe/Quill") keep every matching edge.
  - **Never orphan.** If applying the rule would leave a patch with zero
    ...
    patch named "Product team (Orlin)" fails an exact match, and
