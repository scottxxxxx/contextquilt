#!/usr/bin/env python3
"""
Register the Tech Rehearsal manifest against a running CQ instance.

TR is a structured-ingest app (manifest `ingest_mode: "structured"`): it
sends pre-typed coaching signals, not transcripts. Registration is identical
to any other app — the same admin-gated POST /v1/apps/{app_id}/schema — so
this is a near-clone of register_ss_schema.py pointed at the TR fixture.

Usage:
    # Preview the manifest that would be POSTed (no network call)
    python scripts/register_tr_schema.py --dry-run

    # Check connectivity and app_id validity without registering
    python scripts/register_tr_schema.py --check

    # Register for real
    python scripts/register_tr_schema.py
...
--dry-run only needs the manifest file):
    ...
    TR_APP_ID         the UUID of the Tech Rehearsal application
                      (from GET /v1/auth/apps on the target server)

Prerequisites:
    - registration endpoints live
    - Tech Rehearsal registered as an application (/v1/auth/register)
    - init-db/25_techrehearsal_schema.json present in the repo
...
def load_manifest() -> dict:
    manifest_path = Path(__file__).resolve().parent.parent / "init-db" / "25_techrehearsal_schema.json"
...
def require_env() -> tuple[str, str, str]:
    ...
    app_id = os.environ.get("TR_APP_ID", "").strip()

    missing = []
    if not admin_key:
        missing.append("CQ_ADMIN_KEY")
    if not app_id:
        missing.append("TR_APP_ID")
    if missing:
...
def do_dry_run() -> int:
    ...
    app_id = os.environ.get("TR_APP_ID", "<TR_APP_ID-unset>").strip() or "<TR_APP_ID-unset>"
    ...
    print(f"  ingest_mode:         {manifest.get('ingest_mode')}")
    print(f"  patch_types:         {len(manifest.get('patch_types', []))}")
    print(f"    ({', '.join(pt['domain_type'] for pt in manifest.get('patch_types', []))})")
    longitudinal = [pt['domain_type'] for pt in manifest.get('patch_types', []) if pt.get('longitudinal')]
    print(f"  longitudinal types:  {longitudinal or '(none)'}")
...
def do_register() -> int:
    ...
    print("\nTech Rehearsal schema registered successfully.")
    return 0
