...
Input: "[Scott] I prefer async communication. [Ulster] I'm based in Dallas."
...
WRONG output: identity patch "Ulster based in Dallas" — Ulster is not the (you) speaker
  ...
  "patches": [
    {
      ...
      "connects_to": [
        {"target_text": "Harborview Mutual transcription project", "target_type": "project", "role": "parent", "label": "belongs_to"},
        {"target_text": "Waiting on Trevor to upload audio files", "target_type": "blocker", "role": "depends_on", "label": "blocked_by"}
      ]
    }
  ],
  "entities": [
...
| role       | Someone's role on a project ("Melinda handles escalation")      | YES via belongs_to   |
...
- Person patches carry context about their role on the project (e.g., "Trevor — handles file uploads for Harborview Mutual").
...
- If someone is introduced as "Bramble Martinez" and later called "Bramble", always use "Bramble Martinez"
...
- YES: "Use Nova 3 for Harborview Mutual transcription" — decision with impact
...
TYPE ACCURACY:
- A commitment has a specific NAMED OWNER who promised to DO something. "Someone should finalize the deck" has no owner — that's a takeaway. "Chandran said he'd create the stories" has an owner (Chandran) — that's a commitment.
...
- "[Ulster] I'm based in Dallas" is NOT an identity patch — Ulster is not the (you) speaker.
...
- Only create entities and person patches for people identified by real name (e.g., "Trevor", "Quinby", "Melinda").
