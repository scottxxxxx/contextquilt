...
**GCP VM:** `203.0.113.10` (shared with CloudZap via Project Bifrost)
...
### Model benchmarks (Harborview Mutual transcript):
...
2. SSH into GCP VM (`203.0.113.10`)
...
```
Runs the transcript through the extraction prompt and shows what patches would be created. Uses the Harborview Mutual transcript by default. See also `tests/benchmark/run_pipeline_benchmark.py` for multi-model comparison.
