...
- **Closure ledger (doc 16 §5.10/5.11)**: the dedup re-observation path records WHAT was restated on completables only (`value.restatements` capped 10 + monotonic `value.restatement_count`, `origin_id` as the same-meeting idempotency key; `value.text` and `value.owner` are NEVER rewritten, a handover stamps `value.owner_restated_at`). `services/commitment_ledger.py` is the pure classifier (delivered|absorbed_by_user|reassigned|silently_dropped|re_dated|restated|open, in that precedence, one headline `mode` + all applicable `modes`); `silently_dropped` counts MEETINGS with that person via `person_appearances`, never elapsed days, on the shared `people_signals.is_presence_grade` predicate. Served as `commitment_ledger` on person detail and `commitment_pressure` (unfiltered population, open items only) on the person list. Rules that bind anything built on top: ship the count never the cause, instances never traits, no ratio at any denominator (serve the count and its denominator), `median_hop_count` null never NaN, every count traceable via `patch_ids_by_mode`, and COUNTS ON THE LIST / RECEIPTS ON THE DETAIL (the list strips `commitment_ledger.RECEIPT_KEYS` as a family). `object_regression` is honestly null (seam: `classify_items(regressions=...)`).
- **Chases, doc 16 §5.12**: the follow-up-pressure metric is `chases_without_advance` (a restatement's `origin_id` joined to the same meeting's `questions_from_user_*`, resolved against whether the item CLOSED by the next meeting with that person), NOT questions received. Measured: volume is near level (12 vs 10) across people whose follow up is opposite, so a claim built on volume contradicts the data. Keep the two counts SEPARATE. `ADVANCE_DEFINITION` is on the wire: a re-date and a restatement are NOT advances; `unresolved` (chased in the latest meeting) and `unmeasurable` (meeting predates migration 37) are their own numbers, never folded into failure.
