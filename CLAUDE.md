...
`docs/architecture/00–13` (overview, memory model, pipeline, queue, recall, integration, configuration, API reference, connected quilt, domain mapping, security, model selection, structured ingest, app onboarding/templates, consolidation, context-flow contract, people, prompt composition authority) and `docs/openapi.yaml`. (00–17; 15 is the LOCKED three-team contract — changes to the project-chat context flow require the three-way test per its item 8. 16 is a DRAFT proposal for SS's People object type: nothing in it is implemented, and the person-scoped surface it describes does not exist yet. 17 is the ratified companion to 15: GP owns the prompt composition recipe and SS executes it, so a placement change must never need an SS build; read it before touching anything that decides system block vs user turn.) FastAPI auto-docs at `/docs`. NOTE: docs/openapi.yaml lags the June 2026 surface (meeting views, complete endpoint, token_budget, language) — update when touched.
