...
- Consolidation (`worker.consolidation_loop`, 24h, kill switch `CQ_CONSOLIDATION_ENABLED`): synthesizes higher-order patches from cue-clustered sources per manifest `consolidation_rules` (no rules → inert). Derived patches: `origin_mode='derived'`, `source_patch_ids` (a COLUMN on `context_patches`, NOT a value key, and the lineage every "which evidence fed this card" query must read), `value.source_cue` idempotency stamp, `informs` connections from sources. Insight cards additionally carry evidence in `value.sources` / `value.facts`. See docs/architecture/14-consolidation.md.
...
- Project scope lifecycle: `POST /v1/origins/{u}/{ot}/{oid}/assign-project` (rescope) and its mirrors `.../unassign-project` (optional project_id guard) + `POST /v1/projects/{u}/{pid}/unscope` (project-deletion form; patches survive unscoped). **Meeting delete** is `POST /v1/origins/{u}/{ot}/{oid}/delete` (`services/origin_delete.py`): body `{"delete": true}` archives the origin's patches (self-typed spared, `archive_cause: meeting_deleted`), DELETES its `person_appearances` (the project form only unscopes them), drops its assignment row, then XDELs its transcript LAST; anything else is a preview, 200 always, idempotent. Cleanups archive, never hard-delete (delta-sync tombstone lesson); `scripts/cleanup_orphan_memory.py` (dry-run default).
