...
4. **Build the schema registration API** — `POST /v1/apps/{app_id}/schema` accepts a manifest (patch types + connection labels + extraction guidance + origin_types), validates, writes to `patch_type_registry` and `connection_vocabulary` with `app_id` set.
