...
| Type | Examples |
|------|----------|
| `person` | Bramble Martinez, David Chen |
| `project` | Widget 2.0 |
| `company` | Acme Corp |
...
```
Bramble Martinez --committed_to--> WebSocket prototype
...
Lockridge Chen --leads--> Widget 2.0
```

**Why a graph, not just tags?** Tags tell you "this fact is related to Widget 2.0." The graph tells you "Widget 2.0 has a deadline from Acme Corp, which requires a feature that depends on a prototype Bramble committed to delivering by April 5." The graph enables traversal — starting from any entity and following relationships to surface connected context.
