...
- **Unmatched mentions** — name-shaped mentions in the request text with zero word-level overlap against the entity index (names ∪ aliases). Overlap suppresses the claim: "Sarah" is never reported missing while "Sarah Abrams" is known. Conservative Latin-script heuristic, capped at 3 mentions.
