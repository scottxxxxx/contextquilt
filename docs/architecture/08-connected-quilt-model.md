...
- We can't tell ShoulderSurf "show me the Harborview Mutual quilt" as a connected graph
...
**Total: 8 patches, 5 connections.** That's the entire Harborview Mutual quilt.
...
```
User asks: "catch me up on Harborview Mutual"

1. Match "Harborview Mutual" → project patch
2. Traverse belongs_to connections → find decision, commitment, requirement
3. Traverse blocked_by on commitment → find Trevor action item
...
1. Trevor uploads files → `action_item:Travis_uploads` marked `completed`
...
**Archiving a project:**
1. Harborview Mutual ships → `project:HarborviewMutual` marked `archived`
2. System traverses: find all patches with `belongs_to → project:HarborviewMutual`
  ...
  "patches": [
    {
      ...
      "connects_to": [
        { "target_text": "Harborview Mutual transcription project", "connection": "belongs_to" },
        { "target_text": "Trevor uploads audio files", "connection": "blocked_by" }
      ]
    }
  ],
  "entities": [...],
