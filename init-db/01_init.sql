...
CREATE TABLE IF NOT EXISTS facts (
    ...
    category TEXT,
    app_id TEXT,
    source TEXT DEFAULT 'declared', -- 'declared' (explicit) or 'learned' (LLM-inferred)
    memory_type TEXT DEFAULT 'preference', -- 'identity', 'preference', or 'trait'
    created_at TIMESTAMP
    WITH
        TIME ZONE DEFAULT NOW()
);

-- Insert a dummy user for testing if not exists
INSERT INTO profiles (user_id, variables)
VALUES ('test_user', '{"user_name": "Test User", "seat_preference": "Window", "dietary_restrictions": "Gluten-Free"}'::jsonb)
ON CONFLICT (user_id) DO NOTHING;

-- Applications Table (OAuth2 Client Credentials)
CREATE TABLE IF NOT EXISTS applications (
    app_id UUID PRIMARY KEY DEFAULT gen_random_uuid (),
    app_name VARCHAR(255) NOT NULL,
    client_secret_hash VARCHAR(255) NOT NULL,
    enforce_auth BOOLEAN DEFAULT FALSE,
    created_at TIMESTAMP
    WITH
...
persistence TEXT DEFAULT 'sticky', -- "permanent", "sticky", "ephemeral", "decaying"
