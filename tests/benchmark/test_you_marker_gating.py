...
#     - person:      Ulster (mobile lead, Dallas), Kinsley (PM)
#     - decision:    Use Stripe Connect over building in-house
#     - commitment:  Scott → write RFC by Friday;  Ulster → ship iOS checkout v2 next sprint
...
#     - Kinsley's scheduling action  resolves  compliance-review blocker
#     - Stripe Connect decision  informs  Ulster's iOS commitment
#     - Ulster's iOS commitment   parent of / belongs to  checkout redesign project
SAMPLE_TRANSCRIPT = """[Scott]  Morning everyone. Quick intros since Kinsley just joined.
...
[Ulster]  Hey, I'm Ulster, mobile lead on the same project, working out of Dallas.
[Kinsley]  And I'm Kinsley, the PM — I'll be coordinating the checkout redesign.
[Kinsley]  Okay, the big question on the table: do we build the new payment flow in-house or use Stripe Connect?
[Scott]  I've looked at both. Stripe Connect saves us six months of compliance work. Let's go with Stripe Connect.
[Ulster]  Agreed, that unblocks the iOS side too. I can ship iOS checkout v2 next sprint if the backend is ready.
[Kinsley]  One issue — we haven't scheduled the compliance review for Stripe Connect yet. That's a blocker.
[Kinsley]  I'll get compliance on the calendar this week.
[Scott]  Good. I'll write up the technical RFC by Friday, but I can't finalize it until compliance signs off.
[Ulster]  My iOS work depends on Scott's RFC — I'll start scaffolding the client once it lands.
[Kinsley]  One more thing, for the first milestone we're staying in sandbox only — no production traffic yet.
[Scott]  Got it. Sandbox-only for M1, understood.
[Kinsley]  Thanks everyone — I'll send notes.
"""
...
def summarize_patches(patches: list, label: str) -> dict:
...
async def extract(client: LLMClient, transcript: str) -> list:
...
async def run_model(model: str, base_url: str, api_key: str, without_marker: str, with_marker: str):
    ...
    try:
        patches_off = await extract(client, without_marker)
        patches_on = await extract(client, with_marker)
    finally:
        await client.close()
    return (
        summarize_patches(patches_off, f"{model} — WITHOUT (you)"),
        summarize_patches(patches_on, f"{model} — WITH (you)"),
    )
