...
Attendees: Lockridge Chen (Tech Lead), Marcus Williams (Backend), Kinsley Patel (Frontend).

Lockridge reported that the API rate limiter is deployed to staging and passing load tests at 10,000 requests per second. She plans to roll it out to production tomorrow after the morning traffic peak.

Marcus said he finished the PostgreSQL migration script for the user preferences table. He mentioned there are about 2 million rows to migrate and estimates it will take roughly 45 minutes with the batched approach. He needs Lockridge to review the rollback procedure before he runs it.

Kinsley shared that the new dashboard components are ready for QA. She flagged that the date picker has a timezone bug on Safari that she will fix today.""",

        "expected_facts": [
            {"fact": "Lockridge Chen is the Tech Lead on the Platform team", "category": "identity"},
            {"fact": "Marcus Williams works on backend for the Platform team", "category": "identity"},
            {"fact": "Kinsley Patel works on frontend for the Platform team", "category": "identity"},
        ...
        "expected_action_items": [
            {"action": "Roll out API rate limiter to production after morning traffic peak", "owner": "Lockridge"},
            {"action": "Review rollback procedure for PostgreSQL migration", "owner": "Lockridge"},
            {"action": "Fix Safari timezone bug in date picker", "owner": "Kinsley"},
        ],
    },
    {
...
Attendees: Bramble Martinez (VP Product), Lisa Park (Engineering Director), Tom Reeves (Design Lead), Angela Foster (Sales Director).

Bramble opened by stating that Widget 2.0 needs to ship by end of Q2, which is non-negotiable because three enterprise customers — Acme Corp, TechFlow, and Meridian Systems — have it written into their renewal contracts. He emphasized that losing even one of these accounts would represent over $2M in ARR.
...
Angela sided with Bramble on the timeline but agreed with Tom about offline mode. She revealed that Meridian's CTO specifically mentioned offline support as a deciding factor during their last call. She offered to negotiate with Acme about accepting a beta version of real-time collaboration.

The group agreed on a compromise: ship Widget 2.0 in Q2 with offline mode and real-time collaboration in beta. Angela will call Acme's CTO this week to confirm they accept the beta approach. Bramble said he needs someone to lead the Widget 2.0 project full-time and asked Lisa to identify a candidate by next Monday.""",

        "expected_facts": [
            {"fact": "Bramble Martinez is VP Product", "category": "identity"},
