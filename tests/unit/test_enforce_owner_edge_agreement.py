...
coordinate with Pradeep on turnaround time once IP address is provided"
was stored with owner Pradeep and owns edges from BOTH Pradeep and
Jayanth. Jayanth's real role was supplying the IP address the work waits
...
class TestTheLiveCase:
    ...
    def _content(self) -> dict:
        ...
        return {
            "patches": [
                _commitment(target, "Pradeep"),
                _person("Pradeep", owns=[("commitment", target)]),
                _person("Jayanth", owns=[("commitment", target)]),
            ]
        }

    def test_drops_the_non_owner_edge(self):
        content = enforce_owner_edge_agreement(self._content())
        pradeep, jayanth = content["patches"][1], content["patches"][2]
        assert len(_owns_targets(pradeep)) == 1
        assert _owns_targets(jayanth) == []

    def test_reports_what_it_dropped(self):
        content = enforce_owner_edge_agreement(self._content())
        dropped = content["_owner_edge_agreement_enforced"]["dropped"]
        assert len(dropped) == 1
        assert dropped[0]["person"] == "Jayanth"

    def test_the_person_patch_itself_survives(self):
        """Jayanth was really in the meeting. We drop the claim, not him."""
        content = enforce_owner_edge_agreement(self._content())
        names = [
...
class TestConservatism:
    ...
    def test_compound_owner_keeps_every_edge(self):
        ...
        content = {
            "patches": [
                _commitment("Draft the deck", "Srikanth/Ella"),
                _person("Srikanth", owns=[("commitment", "Draft the deck")]),
                _person("Ella", owns=[("commitment", "Draft the deck")]),
            ]
        }
        after = enforce_owner_edge_agreement(content)
