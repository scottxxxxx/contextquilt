...
def test_owner_keys_covers_canonical_and_aliases():
    assert owner_keys("Sarah Chen", ["Sarah C", "S. C"]) == {
        "sarah chen", "sarah c", "s. c",
    }
...
def test_owner_keys_drops_blank_forms():
    ...
    assert owner_keys("Sarah Chen", ["", "   ", None]) == {"sarah chen"}
...
def test_ledger_owner_must_stay_the_raw_surface_form():
    ...
    owner strings. Resolving "Sarah C" to canonical "Sarah Chen" makes the
