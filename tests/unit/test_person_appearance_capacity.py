...
labels were clean and consistent (Jayanth appears as a label 23 times,
correctly spelled every time) while the same names inside spoken text were
not ("Palavi", "Rashmi", "JN", "JNZ"). Labels are the trustworthy signal;
mention text is not.
"""
...
TRANSCRIPT = """[Suresh] Pradeep. As JN shaves that IP address in the chat here, right?

[Pradeep] I know the URL have changed, but our white justing is done based on IPs.

[Jayanth] I think same thing we need to follow for this also.
...
class TestSpeakerLabelsIn:
    def test_reads_labels_not_body_text(self):
        ...
        assert "suresh" in labels
        assert "pradeep" in labels
        assert "jayanth" in labels

    def test_spoken_nicknames_are_not_speakers(self):
        """'JN' is said out loud inside a Suresh turn. He did not speak."""
...
class TestTheGateScenario:
    ...
    def test_veto_stays_silent_when_only_one_side_spoke(self):
        rows = {}
        merge_tier(rows, {("u", "sukumar", "m"): _row(eid="sukumar")}, "speaker")
        merge_tier(rows, {("u", "sukumar_g", "m"): _row(eid="sukumar_g")}, "mention")
        assert not self._both_spoke(rows, "sukumar", "sukumar_g", "m")
