...
class TestPlaceholder:
    def test_drops_speaker_n(self):
        content = {"patches": [_person("Speaker 5"), _person("Brian")]}
        drop_placeholder_and_self_person_patches(content)
        assert _names(content["patches"]) == ["Brian"]

    def test_drops_speaker_with_paren_real_name(self):
        # "Speaker 2 (Rahul)" is still a placeholder shape — the LLM
        # left the diarization label in. We don't try to rescue the
        # paren name; that's a more complex extraction concern.
        content = {"patches": [_person("Speaker 2 (Rahul)"), _person("Brian")]}
        drop_placeholder_and_self_person_patches(content)
        assert _names(content["patches"]) == ["Brian"]

    def test_drops_unknown(self):
        content = {"patches": [_person("Unknown"), _person("Brian")]}
        drop_placeholder_and_self_person_patches(content)
        assert _names(content["patches"]) == ["Brian"]

    def test_drops_unidentified(self):
        content = {"patches": [_person("Unidentified caller"), _person("Brian")]}
        drop_placeholder_and_self_person_patches(content)
        assert _names(content["patches"]) == ["Brian"]
...
class TestSelfReference:
    def test_drops_user_label_exact(self):
        content = {"patches": [_person("Scott"), _person("Brian")]}
        drop_placeholder_and_self_person_patches(content, user_label="Scott")
        assert _names(content["patches"]) == ["Brian"]

    def test_case_insensitive_match(self):
        content = {"patches": [_person("scott"), _person("SCOTT"), _person("Brian")]}
        drop_placeholder_and_self_person_patches(content, user_label="Scott")
        assert _names(content["patches"]) == ["Brian"]
    ...
    def test_no_user_label_no_self_drop(self):
        ...
        assert _names(content["patches"]) == ["Scott", "Brian"]
...
class TestConnectsToCleanup:
    ...
    def test_keeps_connects_to_referencing_other_names(self):
        action = _action(
            ...
            owner="Brian",
            connects_to=[
                {"target_text": "Brian", "target_type": "person", "role": "informs", "label": "owns"},
            ],
        )
        content = {"patches": [_person("Speaker 5"), _person("Brian"), action]}
        drop_placeholder_and_self_person_patches(content)
        # Speaker 5 dropped; Brian and the action kept; the connect_to to Brian preserved.
        assert _names(content["patches"]) == ["Brian", "Coordinate"]
        assert content["patches"][1]["connects_to"][0]["target_text"] == "Brian"
...
class TestDefensiveEdges:
    ...
    def test_no_match_no_mutation(self):
        ...
        original = [_person("Brian"), _person("Christina")]
