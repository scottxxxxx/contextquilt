...
class TestStripOwnerOnSelfTypedPatches:
    ...
    def test_preserves_owner_on_commitment(self):
        ...
        content = {"patches": [_patch("commitment", "Ship the SDK patch", owner="Brian")]}
        strip_owner_on_self_typed_patches(content)
        assert content["patches"][0]["value"]["owner"] == "Brian"

    def test_preserves_owner_on_blocker(self):
        content = {"patches": [_patch("blocker", "API not exposed", owner="Suresh")]}
        strip_owner_on_self_typed_patches(content)
        assert content["patches"][0]["value"]["owner"] == "Suresh"

    def test_preserves_owner_on_decision(self):
        content = {"patches": [_patch("decision", "Use agent ID for routing", owner="Sai")]}
        strip_owner_on_self_typed_patches(content)
        assert content["patches"][0]["value"]["owner"] == "Sai"
    ...
    def test_mixed_patches_only_self_typed_cleared(self):
        content = {
            "patches": [
                ...
                _patch("commitment", "Ship the SDK", owner="Brian"),
                _patch("preference", "You prefer Slack", owner="Scott"),
                _patch("blocker", "API down", owner="Suresh"),
            ]
        }
        strip_owner_on_self_typed_patches(content)
        owners = [p["value"]["owner"] for p in content["patches"]]
        assert owners == [None, "Brian", None, "Suresh"]
